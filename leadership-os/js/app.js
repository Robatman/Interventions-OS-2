...
// ─── PRACTICE — ALEATORIEDAD ──────────────
// Garantiza personaje y escenario diferentes cada sesión
...
function goBriefing() {
  ...
  if (activeMenu === 'interventions' && currentIntervention) {
    // Intervenciones: usar escenarios de la intervención seleccionada
    pool = getInterventionScenarios(currentLevel);
  } else {
    // Técnicas: mezclar briefings de TODOS los arquetipos
    // para maximizar variedad de personajes y situaciones
  ...
  // ── 3. ARQUETIPO — diferente cada sesión ──────────────
  // Si el escenario especifica agente → usarlo
  // Si no → random, evitando repetir el último
  const lastArchetypeId = window._lastArchetypeId || null;

  if (currentScenario.agentName && ARCHETYPES[currentScenario.agentName]) {
    // El escenario tiene agente definido — respetar el diseño
    currentArchetype = ARCHETYPES[currentScenario.agentName];
  } else {
    // Sin agente definido → completamente aleatorio, no repetir
    currentArchetype = pickRandomArchetype(lastArchetypeId);
  }

  // Guardar para evitar repetición en siguiente sesión
  ...
  if (meta) {
    // Badge del agente + tags del escenario
    const agentBadge = `<span class="briefing-tag" style="background:rgba(0,229,160,0.1);border-color:rgba(0,229,160,0.3);color:#00e5a0;">${currentArchetype.emoji} ${currentArchetype.name}</span>`;
    const tags = currentScenario.tags.map(t => `<span class="briefing-tag">${t}</span>`).join('');
    meta.innerHTML = agentBadge + tags;
  }
